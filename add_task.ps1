...
# Abfrage des Speicherorts fÃ¼r die heruntergeladene Datei
...
# BestÃ¤tigung von Benutzer einholen
$confirm = Read-Host "  | Haben Sie die heruntergeladene Datei Überprüft und bestätigen Sie, dass Sie fortfahren möchten? (y/n)"

if ($confirm -eq "y") {

    # PrÃ¼fen, ob das PSFolderSize-Modul installiert ist und es gegebenenfalls installieren
    $moduleName = "PSFolderSize"
    if (-not (Get-Module -Name $moduleName -ListAvailable)) {
        Write-Host -ForegroundColor Gray "  | Das PowerShell-Modul PSFolderSize muss installiert werden (für ausführliches logging benötigt) "
        Install-Module -Name $moduleName -Scope CurrentUser -Force
    } else {
        Write-Host -ForegroundColor Gray "  | Das PowerShell-Modul PSFolderSize bereits installiert (für ausführliches logging benötigt)"
    }
    ...
    if ($taskExists) {
        ...
        Write-Host -ForegroundColor Gray "  | Die Variable `$logs in der Datei 'auto-delete-folder.ps1' wurde erfolgreich geÃ¤ndert und das Log-Verzeichnis wurde erstellt!"
    }
    else {
